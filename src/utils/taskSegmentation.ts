...
export const getTaskSegments = (
  ...
  if (!task.startDate || !task.endDate) return segments;
  ...
  if (taskEnd < monthStart || taskStart > monthEnd) return segments;

  // Clip to month boundaries
  const visibleStart = Math.max(
    1,
    taskStart.getMonth() === month && taskStart.getFullYear() === year
      ? taskStart.getDate()
      : 1
  );

  const visibleEnd =
    taskEnd.getMonth() === month && taskEnd.getFullYear() === year
      ? taskEnd.getDate()
      : monthEnd.getDate();
